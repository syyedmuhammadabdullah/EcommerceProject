...
const RefundOrderPage = () => {
  ...
  const filters = ["All","Requested", "Approved", "Processing", "Partially Refunded" ,"Refunded","Rejected"];
...
useEffect(() => {
  ...
  const handleFilterChange = (filter) => {
   if (filter!==selectedFilter) {
     dispatch(getSellerOrders({refundStatus:filter,search:debouncedSearch,page:currentPage,limit:10}));
   }
    ...
    <section className="flex justify-center">
      <div className="container lg:gap-xxl  dark:bg-black grid gap-xl px-p-md lg:p-p-xxl">
        ...
        <div className="data  w-full grid overflow-scroll no-scrollbar">
           ...
           <div className="price border pl-[10px] min-w-[137px] flex items-center border-border-primary h-full" >Refund Status</div>
         ...
         {orders?.map((order,index)=>(
            ...
            <div className="id border pl-[10px] w-[48px] flex items-center border-border-primary h-full" >{index+1}</div>
             ...
             <div className="price border pl-[10px] min-w-[137px] flex items-center border-border-primary h-full" >{order.refundStatus}</div>
             <div className="action border pl-[10px] min-w-[140px] flex items-center border-border-primary h-full" >
             <Link to={`/updaterefundstatus/${order._id}`}>
                <Button
                  children="View Order"
