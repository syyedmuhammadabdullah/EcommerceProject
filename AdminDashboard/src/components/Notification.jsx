...
const Notification = () => {
  ...
  const {admin:{_id:adminId}}=useSelector((state) => state.admin);
  ...
  useEffect(() => {
      ...
      if (notifications?.length === 0) {
        dispatch(getNotifications({adminId}));
    }
  }, [dispatch, adminId]);
    ...
    const handleMarkAllAsRead = () => {
        dispatch(markAllNotificationAsRead(adminId));
    }

    const handleClearAll = () => {
        dispatch(clearAllNotifications(adminId));
    }
